...
<div class="card">
    ...
    <div class="card-body p-0">
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('siswa.artikel.show', $item->id_artikel) }}" class="btn btn-info btn-action text-white me-1">Lihat</a>
                            @if(in_array($item->status, ['draft','rejected']))
                            <a href="{{ route('siswa.artikel.edit', $item->id_artikel) }}" class="btn btn-warning btn-action me-1">Edit</a>
                            <form action="{{ route('siswa.artikel.destroy', $item->id_artikel) }}" method="POST" style="display:inline;" onsubmit="return confirm('Hapus artikel ini?')">
                                @csrf @method('DELETE')
        ...
        <div class="text-center py-5 text-muted">
            ...
            <a href="{{ route('siswa.artikel.create') }}" class="btn btn-primary btn-sm">Buat Artikel</a>
        </div>
        @endif
    </div>
</div>
