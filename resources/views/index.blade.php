          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide">
              <div class="blog-hero-item">
                <img src="{{ asset('assets/img/blog/' . $post->gambar) }}" alt="Blog Hero Image" class="img-fluid">
                <div class="blog-hero-content">
                  <span class="category">{{ $post->kategori }}</span>
                  <h1>{{ $post->judul }}</h1>
                  <p class="subtitle">{{ $post->excerpt }}</p>
                  <div class="meta">
                    <span class="author">BY <a href="#">{{ $post->penulis }}</a></span>
                    <span class="date">{{ $post->tanggal_publish }}</span>
                    <span class="read-time">{{ $post->waktu_baca }} Minute Read</span>
                    <span class="views">{{ $post->views }} views</span>
                  </div>
                  <a href="{{ route('blog.details', $post->id) }}" class="read-more">Continue Reading <i class="bi bi-arrow-right"></i></a>
                </div>
              </div>
            </div>
            @endforeach
            ...
            <div class="swiper-slide">
              <article class="blog-card">
                <div class="blog-image">
                  <img src="{{ asset('assets/img/blog/' . $post->gambar) }}" alt="Blog Image" loading="lazy">
                  <div class="category-badge">{{ $post->kategori }}</div>
                </div>
                <div class="blog-content">
                  <div class="author-info">
                    <img src="{{ asset('assets/img/person/person-f-8.webp') }}" alt="Author" class="author-avatar">
                    <div class="author-details">
                      <span class="author-name">{{ $post->penulis }}</span>
                      <span class="publish-date">{{ $post->tanggal_publish }}</span>
                    </div>
                  </div>
                  <h3><a href="{{ route('blog.details', $post->id) }}">{{ $post->judul }}</a></h3>
                  <p>{{ $post->excerpt }}</p>
                  <div class="blog-footer">
                    <div class="reading-time">
                      <i class="bi bi-clock"></i>
                      <span>{{ $post->waktu_baca }} min read</span>
                    </div>
                    <a href="{{ route('blog.details', $post->id) }}" class="btn-read-more">
